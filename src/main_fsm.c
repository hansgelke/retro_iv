...
{
    struct
    fsm_instance* inst = CONTAINER_OF(work, struct fsm_instance, status_work);

    uint8_t gpio_buf20[2] = {0};
    uint8_t intf_buf20[2] = {0};
    i2c_read_register(i2c_bus0, PERIPH_ADDR_20, MCPREG_INTF_B,
                      intf_buf20, sizeof(intf_buf20));


    //LOG_INF("int_flag:[0]=0x%02x level_bit:[0]=0x%02x  FSM Nummer %p", intf_buf20[0], gpio_buf20[0], (void *)inst );
    //Check if Interrupt flag for the Instance is set
    if (intf_buf20[0] & BIT(inst->status_bit))
    {
        //is change flag set?
        //If Flag is set Read GPIO and check if Bit is active or inactive
        //Read will clear interrupt Flag
        //k_sleep(K_MSEC(20));
        i2c_read_register(i2c_bus0, PERIPH_ADDR_20, MCPREG_GPIO_B,
                          gpio_buf20, sizeof(gpio_buf20));

        //Check if GPIO bit is 1 or 0. One means the loop is inactive
        if (gpio_buf20[0] & BIT(inst->status_bit))
        {
            k_event_post(&inst->event, EVENT_BTN_INACTIVE); //Is Det bit cleared or set
            //zero means the loop is active
        }
        else
        {
...
int fsm_init(struct fsm_instance* inst)
{
    ...
    uint8_t gpio_data[] = {0xc0}; //Bit 6 bit 7 enable on change
